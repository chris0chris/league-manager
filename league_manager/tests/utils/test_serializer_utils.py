...
from league_manager.utils.serializer_utils import (
    Obfuscator,
    ObfuscatorSerializer,
    ObfuscateField,
)
...
class TestObfuscatorSerializer:
    def test_obfuscate_field_if_necessary_staff_true(self):
        ...
        assert (
            result == "Firstname"
        ), "When is_staff is True, the value should not be obfuscated"
    ...
    def test_obfuscate_field_if_necessary_staff_false(self):
        ...
        assert (
            result == "L****"
        ), "When is_staff is False, the value should be obfuscated"
...
class TestObfuscateField:
    ...
    def test_to_representation_staff_true(self):
        ...
        parent_serializer.obfuscate_field_if_necessary = MagicMock(
            return_value="SensitiveData"
        )
        field.bind("example_field", parent_serializer)

        result = field.to_representation("SomeValue")

        parent_serializer.obfuscate_field_if_necessary.assert_called_once_with(
            "example_field", "SomeValue"
        )
        assert (
