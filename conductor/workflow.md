...
7. **Verify Before Push:** The full test suite and quality checks (especially LINTING) must be executed and pass locally before pushing changes to any remote branch. **LINTING IS MANDATORY BEFORE PUSH AND DEPLOYMENT.**
...
9. **Lint Before Every Push:** You MUST run the project's linting commands (e.g., `npm run eslint`, `ruff check .`) and resolve ALL issues before every `git push`. Failure to do so violates the project's quality standards.
10. **Upstream Source:** `upstream` is the source for features and issues. Syncing from `upstream` to `origin` or local branches should only be performed when explicitly commanded.
11. **Branch-Per-Track:** Every track MUST be implemented in its own dedicated feature branch. Direct commits to `master` are strictly forbidden.
