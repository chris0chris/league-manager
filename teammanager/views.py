...
def edituser(request, user_id):
    ...
    if request.user.is_superuser | (
        (user_editing.team == user_is_being_edited.team)
        & user_editing.check_teammanager()
    ):
        if request.method == "POST":
            ...
            form = Userform(instance=user_is_being_edited)
        return render(
            request, "editUser.html", {"form": form, "user": user_is_being_edited}
        )
    else:
        return redirect("login")
...
def deleteuser(request, user_id):
    ...
    if request.user.is_superuser | (
        (user_deleting.team == user_is_being_deleted.team)
        & user_deleting.check_teammanager()
    ):
...
def playerdetail(request, player_id):
    ...
    playerAchievements = pd.DataFrame(
        gamedays.models.PlayerAchievement.objects.filter(player=player).values()
    )
    Achievements = pd.DataFrame(gamedays.models.Achievement.objects.all().values())
    if not playerAchievements.empty:
        merge = pd.merge(
            Achievements, playerAchievements, left_on="id", right_on="achievement_id"
        )
        merge.drop(
            ["id_x", "id_y", "player_id", "achievement_id"], axis=1, inplace=True
        )
        game_dict = dict()
        ...
        result = {}
    return render(
        request, "playerDetail.html", {"player": player, "achievements": result}
    )
...
def showachievements(request):
    ...
    achievements = pd.DataFrame(
        gamedays.models.Achievement.objects.all().values()
