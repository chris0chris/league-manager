...
class TestOfficialService(TestCase):
    def test_no_officials_found(self):
        ...
        with pytest.raises(Team.DoesNotExist):
            official_service.get_all_officials_with_team_infos(team_id=1, season=datetime.today().year, is_staff=True)

    def test_officials_for_team_id(self):
        official_service = OfficialService()
        team = DbSetupOfficials().create_officials_full_setup()
        year = datetime.today().year
        result_list = official_service.get_all_officials_with_team_infos(team_id=team.pk, season=year, is_staff=True)
    ...
    def test_officials_for_team_id_and_specific_year(self):
        official_service = OfficialService()
        team = DbSetupOfficials().create_officials_full_setup()
        result_list = official_service.get_all_officials_with_team_infos(team_id=team.pk, season=2020, is_staff=False)
