...
class OfficialSignupService:
    ...
    def create_signup(gameday_id, official_id):
        ...
        if (
            OfficialGamedaySignup.objects.filter(gameday=gameday).count()
            >= OFFICIALS_SIGNUP_LIMIT
        ):
            raise MaxSignupError(
                f"{gameday.name} - Das Limit von {OFFICIALS_SIGNUP_LIMIT} wurde erreicht."
            )
        try:
            OfficialGamedaySignup.objects.create(
                gameday=gameday, official_id=official_id
            )
    ...
    def get_signup_data(official_id, league):
        ...
        relevant_leagues = (
            all_gamedays.order_by().values_list("league__name", flat=True).distinct()
        )
        all_gamedays = (
            all_gamedays.filter(
                Q(league__name=league) if league else Q(),
            )
            .annotate(
