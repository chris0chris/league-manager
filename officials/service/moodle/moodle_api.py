...
class FieldNotFoundException(Exception):
    def __init__(self, user_id, field, current_field):
        super().__init__(
            f"User-Id: {user_id} -> Field '{field}' not found. Instead got {json.dumps(current_field)}"
        )
    ...
    def _get_custom_field_value(self, expected_field_name):
        ...
        if found_entry == {}:
            raise FieldNotFoundException(
                self.id, expected_field_name, self.custom_fields
            )
...
class ApiParticipants:
    ...
    def __init__(self, participants_json):
        all_students = [
            ...
            if any(role["roleid"] == self.STUDENT_ROLE for role in user["roles"])
        ]
        self.participants = []
...
class MoodleApi:
    def __init__(self):
        self.moodle_url = (
            f"{settings.MOODLE_URL}/moodle/webservice/rest/server.php"
    ...
    def confirm_user_auth(self, username, password):
        return MoodleAuth(
            requests.get(
                f"{settings.MOODLE_URL}/moodle/login/token.php",
                {
                    "username": username,
                    "password": password,
    ...
    def update_user(self, api_user: ApiUpdateUser):
        self._send_request(
            f"&wsfunction=core_user_update_users&users[0][id]={api_user.user_id}"
