...
class OfficialGameCountSerializer(OfficialSerializer):
    ...
    def get_position_count(self, obj: Official):
        external_games_by_official: QuerySet[OfficialExternalGames] = (
            obj.officialexternalgames_set.filter(date__year=self.season)
        )
        referee_ext = sum(
            entry.calculated_number_games
            for entry in external_games_by_official.filter(position="Referee")
        )
        down_judge_ext = sum(
            entry.calculated_number_games
            for entry in external_games_by_official.filter(position="Down Judge")
        )
        field_judge_ext = sum(
            entry.calculated_number_games
            for entry in external_games_by_official.filter(position="Field Judge")
        )
        side_judge_ext = sum(
            entry.calculated_number_games
            for entry in external_games_by_official.filter(position="Side Judge")
        )
        mix = sum(
            entry.calculated_number_games
            for entry in external_games_by_official.filter(position="Mix")
        )
        game_officials: QuerySet = obj.gameofficial_set.filter(
            gameinfo__gameday__date__year=self.season
        )
        overall_ext = (
            referee_ext + down_judge_ext + field_judge_ext + side_judge_ext + mix
        )
...
class OfficialGamelistSerializer(OfficialSerializer):
    ...
    def get_external_games(self, obj: Official):
        all_official_entries: QuerySet[OfficialExternalGames] = (
            obj.officialexternalgames_set.filter(date__year=self.season)
        )
        return {
            "all_games": OfficialExternalGamesSerializer(
                instance=all_official_entries, many=True, is_staff=self.is_staff
            ).data,
            "number_games": sum(
                entry.calculated_number_games for entry in all_official_entries
            ),
            "last_update": OfficialExternalGames.objects.all().aggregate(
                Max("notification_date")
    ...
    def get_dffl_games(self, obj: Official):
        ...
        return {"all_games": all_games, "number_games": len(all_games)}
...
class GameOfficialAllInfoSerializer(Serializer):
    ...
    ALL_VALUE_FIELDS = [
        ...
        GAMEDAY_DATE_C,
        GAMEDAY_NAME_C,
        GAMEINFO_OFFICIALS_ID_C,
        GAMEINFO_OFFICIALS_TEAM_DESCRIPTION_C,
        GAMEINFO_OFFICIALS_TEAM_NAME_C,
        OFFICIAL_FIRST_NAME_C,
        OFFICIAL_LAST_NAME_C,
        OFFICIAL_TEAM_NAME_C,
    ]
    ...
    def obfuscate_name(self, obj: dict):
        ...
        if (
            self.display_names_for_team == obj[self.GAMEINFO_OFFICIALS_TEAM_NAME_C]
            or self.display_names_for_team == obj[self.OFFICIAL_TEAM_NAME_C]
            or self.is_staff
        ):
    ...
    def _get_official_name(self, game_official: dict) -> dict:
        ...
        return {"is_mapped": False, "name": game_official.get(self.NAME_C)}
