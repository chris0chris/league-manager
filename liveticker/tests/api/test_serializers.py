...
class TestTeamlogSerializer:
    ...
    def test_default_works(self):
        now = datetime.now()
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(player=7, created_time=now),
                           TestTeamlogSerializer.create_some_teamlog()]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team='name of team', many=True)
    ...
    def test_team_is_none(self):
        now = datetime.now()
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(event='Spielzeit', input='1:05')]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team='name of team', many=True)
        assert dict(serializer.data[0]) == {
            "team": None,
            "text": "Spielzeit - 1:05",
    ...
    def test_get_text_for_pat(self):
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(player=7, event='1-Extra-Punkt')]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team=None, many=True)
        assert serializer.data[0]['text'] == '1-Extra-Punkt: #7'

    def test_get_text_for_timeout(self):
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(event='Auszeit', input='00:01')]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team=None, many=True)
        assert serializer.data[0]['text'] == 'Auszeit - 00:01'

    def test_get_text_for_incomplete_pat(self):
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(event='1-Extra-Punkt')]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team=None, many=True)
        assert serializer.data[0]['text'] == '1-Extra-Punkt: -'

    def test_get_text_for_safety(self):
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(player=7, event='Safety')]
        serializer = TeamlogSerializer(instance=teamlog_entries, home_team=None, many=True)
    ...
    def test_is_home_or_way_in_possession(self):
        teamlog_entries = [TestTeamlogSerializer.create_some_teamlog(),
...
class TestLivetickerSerializer:
    ...
    def create_some_liveticker(**kwargs):
        allowed_keys = [
            'scheduled', 'id', 'status', 'standing', 'gameStarted',
            'name_home', 'name_away', 'score_home', 'score_away',
            'in_possession', 'teamlog', 'full_name_home', 'full_name_away'
        ]
        defaults = {
    ...
    def test_game_with_no_team_logs(self):
        liveticker_entries = [TestLivetickerSerializer.create_some_liveticker(teamlog=[])]
    ...
    def test_game_with_game_started_time(self):
        ...
        liveticker_entries = [TestLivetickerSerializer.create_some_liveticker(gameStarted=now)]
        serializer = LivetickerSerializer(instance=liveticker_entries, many=True)
