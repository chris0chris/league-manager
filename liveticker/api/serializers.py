...
class LivetickerSerializer(Serializer):
    ...
    TEAMLOG = 'teamlog'
    ALL_VALUE_FIELDS = ['status', 'standing', NAME_HOME, NAME_AWAY, SCORE_HOME, SCORE_AWAY, IN_POSSESSION, ID,
                        FULL_NAME_HOME, FULL_NAME_AWAY, SCHEDULED, GAME_STARTED]
    ...
    def get_ticks(self, obj: dict):
        return TeamlogSerializer(instance=obj[self.TEAMLOG], home_team=obj[self.NAME_HOME], many=True).data
    ...
    def get_home(self, obj: dict):
        return self._get_team_dict(obj[self.FULL_NAME_HOME], obj[self.NAME_HOME], obj[self.SCORE_HOME],
                                   obj[self.IN_POSSESSION])

    def get_away(self, obj: dict):
        return self._get_team_dict(obj[self.FULL_NAME_AWAY], obj[self.NAME_AWAY], obj[self.SCORE_AWAY],
                                   obj[self.IN_POSSESSION])

    # noinspection PyMethodMayBeStatic
    def _get_team_dict(self, full_team_name, team_name: str, score: int, in_possession: str):
        return {
            'name': full_team_name,
            'score': score,
