...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for schedule_file in schedule_files:
            ...
            if dry_run:
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"  Failed to migrate schedule_{format_name}: {e}"
                        )
                    )
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(
                    f"DRY RUN: Would migrate {len(schedule_files)} template(s)"
                )
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully migrated {migrated_count} template(s)"
                )
            )
    ...
    def _get_schedule_files(
        self, schedules_dir: pathlib.Path, formats: Optional[List[str]]
    ) -> List[pathlib.Path]:
        ...
        if formats:
            ...
            for fmt in formats:
                ...
                if schedule_file.exists():
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"Schedule file not found: {schedule_file.name}"
                        )
                    )
            return files
        else:
    ...
    def _load_schedule_json(
        self, schedules_dir: pathlib.Path, format_name: str
    ) -> List[Dict]:
    ...
    def _create_slots(self, template: ScheduleTemplate, schedule_data: List[Dict]):
        ...
        for field_entry in schedule_data:
            ...
            for slot_order, game in enumerate(games):
                ...
                home_group, home_team, home_ref = self._parse_team_for_slot(
                    game.get("home", "")
                )
                away_group, away_team, away_ref = self._parse_team_for_slot(
                    game.get("away", "")
                )
                official_group, official_team, official_ref = self._parse_team_for_slot(
                    game.get("official", "")
                )
    ...
    def _create_update_rules(
        self, schedules_dir: pathlib.Path, template: ScheduleTemplate, format_name: str
    ):
    ...
    def _create_team_rule(
        self, update_rule: TemplateUpdateRule, role: str, team_data: Dict
    ):
