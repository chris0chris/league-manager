...
class TemplateSlotInline(admin.TabularInline):
    ...
    extra = 1
    fields = (
        'field', 'slot_order', 'stage', 'standing',
        'home_group', 'home_team', 'home_reference',
        'away_group', 'away_team', 'away_reference',
        'official_group', 'official_team', 'official_reference',
        'break_after'
    )
...
class TemplateUpdateRuleInline(admin.StackedInline):
    ...
    inlines = [TemplateUpdateRuleTeamInline]  # Note: Nested inlines not supported in Django admin by default
    can_delete = True
...
class ScheduleTemplateAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Template Information', {
            'fields': ('name', 'description', 'association')
        }),
...
class TemplateSlotAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Game Information', {
            'fields': ('stage', 'standing', 'break_after')
        }),
        ('Home Team', {
    ...
    def teams_display(self, obj):
        ...
        home = f"{obj.home_group}_{obj.home_team}" if obj.home_group is not None else obj.home_reference
        away = f"{obj.away_group}_{obj.away_team}" if obj.away_group is not None else obj.away_reference
...
class TemplateUpdateRuleAdmin(admin.ModelAdmin):
    ...
    def slot_display(self, obj):
        ...
        return f"Field {obj.slot.field}, Slot {obj.slot.slot_order} ({obj.slot.standing})"
...
class TemplateUpdateRuleTeamAdmin(admin.ModelAdmin):
    ...
    fields = ('update_rule', 'role', 'standing', 'place', 'points', 'pre_finished_override')
...
class TemplateApplicationAdmin(admin.ModelAdmin):
    ...
    readonly_fields = ('template', 'gameday', 'applied_at', 'applied_by', 'team_mapping')
