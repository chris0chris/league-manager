    ...
    class Meta:
        constraints = [
            ...
            models.CheckConstraint(
                condition=Q(num_teams__gt=0),
                name='positive_num_teams'
            ),
            models.CheckConstraint(
                condition=Q(num_fields__gt=0),
                name='positive_num_fields'
            ),
            models.CheckConstraint(
...
class TemplateSlot(models.Model):
    ...
    break_after = models.PositiveIntegerField(default=0)  # Additional break time in minutes
    ...
    def clean(self):
        ...
        if (self.home_group is not None and self.away_group is not None and
            self.home_group == self.away_group and
            self.home_team == self.away_team):
            errors['home_team'] = "Team cannot play against itself"

        # Validate no self-referee (home team cannot referee own game)
        if (self.home_group is not None and self.official_group is not None and
            self.home_group == self.official_group and
            self.home_team == self.official_team):
            errors['official_team'] = "Home team cannot referee their own game"

        # Validate no self-referee (away team cannot referee own game)
        if (self.away_group is not None and self.official_group is not None and
            self.away_group == self.official_group and
            self.away_team == self.official_team):
        ...
        if self.template:
            if self.home_group is not None and self.home_group >= self.template.num_groups:
                errors['home_group'] = (
                    f"Home group {self.home_group} exceeds template's {self.template.num_groups} groups"
                )

            if self.away_group is not None and self.away_group >= self.template.num_groups:
                errors['away_group'] = (
                    f"Away group {self.away_group} exceeds template's {self.template.num_groups} groups"
                )

            if self.official_group is not None and self.official_group >= self.template.num_groups:
    ...
    def __str__(self):
        home_desc = f"{self.home_group}_{self.home_team}" if self.home_group is not None else self.home_reference
        away_desc = f"{self.away_group}_{self.away_team}" if self.away_group is not None else self.away_reference
...
class TemplateApplication(models.Model):
    ...
    def __str__(self):
        return f'{self.template.name} applied to {self.gameday.name} on {self.applied_at}'
