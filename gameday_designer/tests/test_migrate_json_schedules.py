...
class MigrateJsonSchedulesCommandTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.schedules_dir = pathlib.Path(__file__).parent.parent.parent / 'gamedays' / 'management' / 'schedules'
    ...
    def test_command_exists(self):
        ...
        call_command('migrate_json_schedules', '--dry-run', stdout=self.stdout, stderr=self.stderr)
    ...
    def test_format_filter_single_format(self):
        ...
        call_command('migrate_json_schedules', '--format', '6_2', '--dry-run', stdout=self.stdout, stderr=self.stderr)
        output = self.stdout.getvalue()
    ...
    def test_creates_template_from_simple_schedule(self):
        ...
        call_command('migrate_json_schedules', '--format', '2_1', stdout=self.stdout, stderr=self.stderr)
    ...
    def test_creates_slots_from_schedule_json(self):
        ...
        call_command('migrate_json_schedules', '--format', '6_2', stdout=self.stdout, stderr=self.stderr)
    ...
    def test_parses_team_assignment_reference_string(self):
        ...
        final_slot = template.slots.filter(
            stage='Finalrunde',
            home_reference__icontains='Gruppe'
        ).first()
    ...
    def test_update_rule_team_points_filter(self):
        ...
        team_rule_with_points = TemplateUpdateRuleTeam.objects.filter(
            update_rule__template=template,
    ...
    def test_update_rule_team_pre_finished_override(self):
        ...
        team_rule_with_override = TemplateUpdateRuleTeam.objects.filter(
            update_rule__template=template,
            pre_finished_override__isnull=False
        ).first()
    ...
    def test_field_number_parsed_correctly(self):
        ...
        call_command('migrate_json_schedules', '--format', '8_2', stdout=self.stdout, stderr=self.stderr)
