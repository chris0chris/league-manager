...
class TemplateApplicationHelper:
    ...
    @staticmethod
    def apply_template_to_gameday(
        template: ScheduleTemplate, gameday: Gameday, team_groups: List[List[Team]]
    ):
        """
...
class TestScheduleMigrationValidation(TestCase):
    ...
    def _create_gameday_from_json(
        self, format_name: str, team_groups: List[List[Team]]
    ) -> Gameday:
        ...
        for idx, teams in enumerate(team_groups):
            ...
            group_schedules.append(
                GroupSchedule(name=group_name, league_group=None, teams=teams)
            )
    ...
    def _create_gameday_from_template(
        self, format_name: str, team_groups: List[List[Team]]
    ) -> Gameday:
    ...
    def _compare_gamedays(self, gameday1: Gameday, gameday2: Gameday):
        ...
        """
        gameinfos1 = list(
            Gameinfo.objects.filter(gameday=gameday1).order_by("field", "scheduled")
        )
        gameinfos2 = list(
            Gameinfo.objects.filter(gameday=gameday2).order_by("field", "scheduled")
        )
        ...
        for i, (info1, info2) in enumerate(zip(gameinfos1, gameinfos2)):
            with self.subTest(game_index=i):
                ...
                self.assertEqual(
                    info1.scheduled,
                    info2.scheduled,
                    f"Game {i}: Different scheduled time",
                )
                ...
                # Standing
                self.assertEqual(
                    info1.standing, info2.standing, f"Game {i}: Different standing"
                )

                # Officials
                self.assertEqual(
                    info1.officials, info2.officials, f"Game {i}: Different officials"
                )

                # Teams (home and away)
                results1 = list(
                    Gameresult.objects.filter(gameinfo=info1).order_by("-isHome")
                )
                results2 = list(
                    Gameresult.objects.filter(gameinfo=info2).order_by("-isHome")
                )
                ...
                # Home team
                self.assertEqual(
                    results1[0].team, results2[0].team, f"Game {i}: Different home team"
                )

                # Away team
                self.assertEqual(
                    results1[1].team, results2[1].team, f"Game {i}: Different away team"
                )
    ...
    def _test_json_schedule_creates_gameday(self, format_name):
        ...
        self.assertGreater(
            gameinfos.count(), 0, f"Format {format_name}: No games created"
        )
        ...
        for gameinfo in gameinfos:
            ...
            self.assertEqual(
                results.count(), 2, f"Format {format_name}: Game should have 2 teams"
            )
...
class TestMigrationCoverage(TestCase):
    ...
    def test_migration_creates_all_templates(self):
        ...
        for format_name in SCHEDULE_FORMATS:
            ...
            self.assertTrue(
                template.exists(), f"Template not created for format: {format_name}"
            )
    ...
    def test_template_has_slots(self):
        ...
        for format_name in SCHEDULE_FORMATS:
            with self.subTest(format=format_name):
                ...
                self.assertGreater(
                    template.slots.count(), 0, f"Format {format_name}: No slots created"
                )
        ...
        for format_name in SCHEDULE_FORMATS:
            with self.subTest(format=format_name):
                ...
                for slot in template.slots.all():
                    ...
                    has_home_indices = (
                        slot.home_group is not None and slot.home_team is not None
                    )
                    ...
                    has_away_indices = (
                        slot.away_group is not None and slot.away_team is not None
                    )
                    ...
                    has_official_indices = (
                        slot.official_group is not None
                        and slot.official_team is not None
                    )
