...
from gamedays.api.serializers import LeagueManagerSerializer, GamedayManagerSerializer, TeamManagerSerializer
from gamedays.models import LeagueManager, GamedayManager, TeamManager, Team, League, Season, SeasonLeagueTeam
...
class GamedayManagerUpdateDeleteAPIView(APIView):
    ...
    def patch(self, request, pk):
        ...
        if not (request.user.is_staff or
                ManagerPermissionHelper.is_league_manager(
...
class TeamManagerListCreateAPIView(APIView):
    ...
    def get(self, request, team_id):
        ...
        if not (request.user.is_staff or
                ManagerPermissionHelper.can_assign_team_manager(request.user, team) or
                ManagerPermissionHelper.is_team_manager(request.user, team)):
    ...
    def post(self, request, team_id):
        ...
        team = Team.objects.get(pk=team_id)
        if not (request.user.is_staff or ManagerPermissionHelper.can_assign_team_manager(request.user, team)):
...
class TeamManagerDeleteAPIView(APIView):
    ...
    def delete(self, request, pk):
        ...
        # Check permissions
        if not (request.user.is_staff or
                ManagerPermissionHelper.can_assign_team_manager(request.user, manager.team)):
            raise PermissionDenied("League manager or staff permission required")
...
class UserManagerPermissionsAPIView(APIView):
    ...
    def get(self, request):
        ...
        if not user.is_staff:
            ...
            league_managers = LeagueManager.objects.filter(user=user).select_related('league', 'season')
            ...
            gameday_managers = GamedayManager.objects.filter(user=user).select_related('gameday')
