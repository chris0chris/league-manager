...
from gamedays.management.schedule_update import (
    ScheduleUpdate,
    UpdateGameEntry,
    UpdateEntry,
)
...
class StandingWrapper:
    ...
    def is_updated_as_expected_with(
        self, expected_home_team, expected_away_team, expected_official_team
    ):
        ...
        assert (
            self.gameinfo.officials.name == expected_official_team
        ), "official team doesnt match expected"


def check_if_first_standing(standing):
    return StandingWrapper(
        Gameinfo.objects.filter(standing=standing).exclude(status="beendet").first()
    )


def update_gameresults_by_standing_and_finish_game_for(standing: str):
    games_for_standing = Gameinfo.objects.filter(standing=standing).exclude(
        status="beendet"
    )
...
def update_gameresults_and_finish_first_game_for_P7():
    game: Gameinfo = (
        Gameinfo.objects.filter(standing="P7").exclude(status="beendet").first()
    )
    update_gameresults(game)
    game.status = "beendet"
    game.save()
...
class TestScheduleUpdate(TransactionTestCase):
    ...
    def test_update_9_teams_3_fields(self):
        ...
        groups = [
            ...
            GroupSchedule(name="Gruppe 3", league_group=None, teams=group_C),
        ]
        DBSetup().create_playoff_placeholder_teams()
        sc = ScheduleCreator(
            gameday=Gameday.objects.get(pk=gameday.pk),
    ...
    def test_officials_update(self):
        gameday = DBSetup().g62_qualify_finished()
        games = Gameinfo.objects.filter(standing="HF") | Gameinfo.objects.filter(
            standing="P5"
        )
        sf1 = 0
        sf2 = 1
...
class TestUpdateGameEntry:
    ...
    def test_get_methods(self):
        uge = UpdateGameEntry(
            {
                "home": {"standing": "Gruppe 1", "place": 3},
                "away": {"stage": "Vorrunde", "place": 0, "index": 1},
                "officials": {
                    "pre_finished": "HF",
                    "standing": "HF",
                    "points": 2,
