...
class TestSignalsDualLogic(TestCase):
    ...
    @patch(
        "gamedays.service.schedule_resolution_service.GamedayScheduleResolutionService.update_participants"
    )
    def test_signal_uses_legacy_logic_when_no_template_application(
        self, res_service_mock, update_mock
    ):
    ...
    def test_signal_uses_designer_logic_when_template_application_exists(
        self, is_finished_mock, res_service_mock, update_mock
    ):
        ...
        TemplateApplication.objects.create(
            gameday=self.gameday, template=template, team_mapping={}
        )
