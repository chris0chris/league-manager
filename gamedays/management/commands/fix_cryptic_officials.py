...
in-place. If a ``Team`` with the resolved name already exists (``Team.name`` is
globally unique — this happens when the same placeholder was already resolved
correctly elsewhere, e.g. as a home/away team in the same gameday), renaming
would violate that uniqueness constraint. In that case the command instead
repoints the referencing ``Gameinfo.officials`` to the existing team and
leaves the cryptic ``Team`` row unrenamed (still no deletes).
...
* Only touches ``Team.name`` (rename) or ``Gameinfo.officials`` (repoint on
  name collision) for rows matching the cryptic pattern — no deletes, no
  cascade risk.
"""
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        self.stdout.write(self.style.WARNING(f"=== fix_cryptic_officials [{mode}] ==="))
        ...
        if gameday_id:
            ...
            gameinfo_ids = list(
                Gameinfo.objects.filter(gameday_id=gameday_id).values_list(
                    "officials_id", flat=True
                )
            )
        ...
        if not cryptic_teams:
            self.stdout.write(
                self.style.SUCCESS("No cryptic official teams found. Nothing to do.")
            )
            return
        ...
        for team in cryptic_teams:
            ...
            for gi in referencing_gis:
                ...
                standing = self._find_standing_for_uuid(canvas_state, game_uuid)
                if not standing:
                    unresolvable.append(
                        (
                            team.id,
                ...
                if new_name:
                    changes.append(
                        (team.id, team.name, new_name, f"gameday {gi.gameday_id}")
                    )
                    resolved = True
            ...
            try:
                with transaction.atomic():
                    for team_id, _old, new_name, _source in changes:
                        # Team.name is globally unique. If another Team row
                        # already has the resolved name (e.g. it was already
                        # created correctly as a home/away team elsewhere in
                        # the same gameday), renaming in place would collide.
                        # Repoint the referencing Gameinfo(s) to that existing
                        # team instead, mirroring CanvasPublishService's
                        # get_or_create-by-name reuse. The cryptic row is left
                        # in place, unrenamed — no deletes.
                        existing = (
                            Team.objects.filter(name=new_name)
                            .exclude(id=team_id)
                            .first()
                        )
                        if existing:
                            ...
                            self.stdout.write(
                                ...
                                f"as team {existing.id} — repointed officials "
                                "instead of renaming."
                            )
                        else:
    ...
    def _print_results(self, changes, unresolvable):
        if changes:
            ...
            self.stdout.write(
                f"  {'Team ID':>8}  {'Old Name':<35}  {'New Name':<30}  Source"
            )
