...
class PasscheckMenu(BaseMenu):
    ...
    def get_menu_items(self, request):
        ...
        if request.user.is_authenticated:
            default_menu.append(
                MenuItem.create(name="Player erstellen", url=PASSCHECK_PLAYER_CREATE)
            )
        default_menu.extend(
            [
                MenuDivider.create(),
                MenuItem.create(name="Scorecard / Passcheck", url=PASSCHECK_APP),
            ]
        )
        return default_menu
