...
class DbSetupPasscheck:
    @staticmethod
    def create_playerlist_for_team(
        team=None, gamedays=()
    ) -> tuple[Team, Playerlist, Playerlist, Playerlist]:
        if team is None:
            team = TeamFactory(name="Passcheck team")
        if len(gamedays) == 0:
            gamedays = [GamedayFactory(), GamedayFactory()]
        today = datetime.today()
        female = PlayerlistFactory(
            team=team,
            jersey_number=7,
    ...
    @staticmethod
    def create_playerlist_for_team_with_eligibility_rule() -> (
        tuple[League, Season, Team]
    ):
        prime_league, second_league, third_league, season, team = (
        ...
        DbSetupPasscheck.create_playerlist_for_team(
            team, [prime_gameday, third_league_gameday]
        )
