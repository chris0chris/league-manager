...
class TestPasscheckService(TestCase):
    ...
    def test_entries_are_deleted_and_created(self):
        ...
        passcheck_service_players = PasscheckServicePlayers()
        PlayerlistGameday.objects.create(playerlist=female, gameday=gameday, gameday_jersey=99)
        passcheck_service_players.create_roster_and_passcheck_verification(team_id=team.pk, user=user,
    ...
    def test_passcheck_verification_is_updated(self):
        ...
        gameday = DBSetup().create_empty_gameday()
        user = User.objects.first()
        PasscheckVerification.objects.create(team=team, gameday=gameday, user=user,
                                             official_name='Initial official name')
        passcheck_service_players = PasscheckServicePlayers()
        passcheck_service_players.create_roster_and_passcheck_verification(team_id=team.pk, gameday_id=gameday.pk,
                                                                           user=user, data={
                'official_name': 'Verified Official',
