...
from passcheck.urls import (
    PASSCHECK_PLAYER_GAMES_LIST,
    PASSCHECK_ROSTER_LIST,
    PASSCHECK_PLAYER_CREATE,
    PASSCHECK_ROSTER_UPDATE,
    PASSCHECK_ROSTER_TRANSFER,
    PASSCHECK_TRANSFER_LIST,
)
...
class TestPlayerlistDeleteView(WebTest):
    def test_display_team_roster(self):
        ...
        response: DjangoWebtestResponse = self.app.get(
            reverse(PASSCHECK_ROSTER_DELETE, kwargs={"pk": female.pk})
        )
...
class TestPlayerlistUpdateView(WebTest):
    def test_update_player(self):
        ...
        self.app.set_user(user)
        response: DjangoWebtestResponse = self.app.get(
            reverse(PASSCHECK_ROSTER_UPDATE, kwargs={"pk": player.pk})
        )
        assert response.status_code == HTTPStatus.OK
        form = response.forms[1]
        ...
        assert response.request.path == reverse(
            PASSCHECK_ROSTER_LIST, kwargs={"pk": team.pk}
        )
...
class TestPlayerlistTransferView(WebTest):
    ...
    def test_init_transfer(self):
        ...
        self.app.set_user(user)
        response: DjangoWebtestResponse = self.app.get(
            reverse(PASSCHECK_ROSTER_TRANSFER, kwargs={"pk": player.pk})
        )
        assert response.status_code == HTTPStatus.OK
        form = response.forms[1]
        assert "disabled" in form.fields["first_name"][0].attrs
        form["new_team"].force_value([receiving_team.pk])
        response: DjangoWebtestResponse = form.submit(
            "action_type", value="pending"
        ).follow()
        assert response.status_code == HTTPStatus.OK
