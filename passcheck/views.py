...
def build_message(action_msg, form):
    msg = (
        f"Player {action_msg}: #{form.instance.jersey_number} "
...
class PlayerlistDeleteView(UserPassesTestMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse(PASSCHECK_ROSTER_LIST, kwargs={"pk": playerlist.team_id})
        )  # Redirect to a safe URL

    def test_func(self):
        playerlist = self.get_object(pk=self.kwargs.get("pk"))
        return PermissionHelper.has_staff_or_user_permission(
            self.request, playerlist.team_id
        )
...
class PlayerlistTransferView(PlayerlistCommonMixin, UserPassesTestMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["has_transfer_status"] = (
            context["object"].playerlisttransfer_set.filter(status="pending").exists()
        )
        return context
    ...
    def test_func(self):
        ...
        if PlayerlistTransfer.objects.filter(
...
class PasscheckPlayerGamesList(View):
    ...
    def get(self, request, **kwargs):
        ...
        return render(
            request,
            self.template_name,
            passcheck_service.get_player_gamedays(player_id),
        )
...
class TransferListView(ListView):
    ...
    def get_queryset(self):
        ...
        if not user.is_staff:
            for obj in queryset:
                obj.current_team.player.person.first_name = Obfuscator.obfuscate(
                    obj.current_team.player.person.first_name
                )
                obj.current_team.player.person.last_name = Obfuscator.obfuscate(
                    obj.current_team.player.person.last_name
                )
