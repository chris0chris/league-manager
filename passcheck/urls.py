...
from .views import (
    PasscheckPlayerGamesList,
    PlayerlistCreateView,
    RosterView,
    PasscheckView,
    PlayerlistUpdateView,
    PlayerlistTransferView,
    TransferListView,
    PlayerlistDeleteView,
)
...
urlpatterns = [
    ...
    path(
        ...
        name=PASSCHECK_LIST_FOR_ALL_TEAMS,
        kwargs={"app": "passcheck"},
    ),
    path(
        "team/not-existent/",
        TemplateView.as_view(template_name="passcheck/team-not-existent.html"),
        name=PASSCHECK_TEAM_NOT_EXISTENT,
    ),
    path(
        "player/<int:pk>/games/list/<int:year>/",
        PasscheckPlayerGamesList.as_view(),
        name=PASSCHECK_PLAYER_GAMES_LIST,
    ),
    path(
        "player/<int:pk>/games/list/",
        PasscheckPlayerGamesList.as_view(),
        name=PASSCHECK_PLAYER_GAMES_LIST,
    ),
    path(
        "player/create/", PlayerlistCreateView.as_view(), name=PASSCHECK_PLAYER_CREATE
    ),
    path(
        "player/<int:pk>/update/",
        PlayerlistUpdateView.as_view(),
        name=PASSCHECK_ROSTER_UPDATE,
    ),
    path(
        "player/<int:pk>/delete/",
        PlayerlistDeleteView.as_view(),
        name=PASSCHECK_ROSTER_DELETE,
    ),
    path(
        "player/<int:pk>/transfer/",
        PlayerlistTransferView.as_view(),
        name=PASSCHECK_ROSTER_TRANSFER,
    ),
    ...
    path(
        ...
        RosterView.as_view(),
        name=PASSCHECK_ROSTER_LIST_FOR_YEAR,
    ),
]
