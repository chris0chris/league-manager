...
from passcheck.service.passcheck_service import (
    PasscheckService,
    PasscheckServicePlayers,
    PasscheckException,
)
...
class PasscheckGamesAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            return Response(
                passcheck.get_passcheck_games(team_id=team_id, gameday_id=gameday_id),
...
class PasscheckApprovalUrlAPIView(APIView):
    ...
    def get(self, request, **kwargs):
        ...
        try:
            ...
            raise NotFound(
                detail=f"Keine Equipment-Genehmigung gefunden für das Team: {team_id}"
            )
        except PermissionError:
            raise PermissionDenied(
                detail=f"Kein Zugriff auf die Equipment-Genehmigung. Bitte an Admin wenden (403)."
            )
...
class PasscheckGamesStatusAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        passcheck = PasscheckService(user_permission=user_permission)
        return Response(
            passcheck.get_passcheck_status(officials_team=request.user.username),
            status=HTTPStatus.OK,
        )
...
class PasscheckRosterAPIView(APIView):
    ...
    def put(self, request, **kwargs):
        ...
        passcheckservice.create_roster_and_passcheck_verification(
            team_id, gameday_id, request.user, data
        )
